...
export default function TokenInteract() {
  ...
  const [formData, setFormData] = useState({
    ...
    contractAddress: TOKEN_STANDARDS.ERC20.defaultAddress,
    contractAbi: TOKEN_STANDARDS.ERC20.defaultAbi,
    privateKey: "",
  ...
  const validateInputs = () => {
    ...
    // 只有在非只读函数时才需要私钥
    if (selectedFunctionDetails && !selectedFunctionDetails.isReadOnly && !formData.privateKey) {
  ...
  const getResponseFromContract = async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(
        ...
        selectedFunctionDetails.isReadOnly ? contractState.provider : contractState.signer
      );
  ...
  return (
    <Card title="代币合约交互" style={{ maxWidth: 800, margin: '0 auto' }}>
